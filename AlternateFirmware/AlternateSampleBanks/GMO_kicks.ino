...
#include <samples/Wavetables/kickOne_int8.h> // Table for Sample
#include <samples/Wavetables/kickTwo_int8.h> // Table for Sample 
#include <samples/Wavetables/kickThree_int8.h> // Table for Sample 
#include <samples/Wavetables/kickFour_int8.h> // Table for Sample 
#include <samples/Wavetables/kickFive_int8.h> // Table for Sample 
#include <samples/Wavetables/kickSix_int8.h> // Table for Sample
#include <samples/Wavetables/kickSeven_int8.h> // Table for Sample 
#include <samples/Wavetables/kickEight_int8.h> // Table for Sample
#include <samples/Wavetables/kickNine_int8.h> // Table for Sample
#include <samples/Wavetables/kickTen_int8.h> // Table for Sample

#define CONTROL_RATE 256 // powers of 2 please 

Sample <KICKONE_NUM_CELLS, AUDIO_RATE> aSample(KICKONE_DATA); // create the wavetable template 
...
*/
  ...
  */
  ...
  pitch = mapfloat(pitchMath, 1, 1023, 0.25, 3.0); // map from 0.01% to triple speed
  ...
  */
  ...
  switch (sampleSelect) {
    case 1:
      aSample.setTable(KICKONE_DATA); // change the current wavetable
      CURRENT_TABLE = KICKONE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 2:
      aSample.setTable(KICKTWO_DATA); // change the current wavetable
      CURRENT_TABLE = KICKTWO_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 3:
      aSample.setTable(KICKTHREE_DATA); // change the current wavetable
      CURRENT_TABLE = KICKTHREE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 4:
      aSample.setTable(KICKFOUR_DATA); // change the current wavetable
      CURRENT_TABLE = KICKFOUR_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 5:
      aSample.setTable(KICKFIVE_DATA); // change the current wavetable
      CURRENT_TABLE = KICKFIVE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 6:
      aSample.setTable(KICKSIX_DATA); // change the current wavetable
      CURRENT_TABLE = KICKSIX_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 7:
      aSample.setTable(KICKSEVEN_DATA); // change the current wavetable
      CURRENT_TABLE = KICKSEVEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 8:
      aSample.setTable(KICKEIGHT_DATA); // change the current wavetable
      CURRENT_TABLE = KICKEIGHT_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 9:
      aSample.setTable(KICKNINE_DATA); // change the current wavetable
      CURRENT_TABLE = KICKNINE_NUM_CELLS; // set the wavetable length to current wavetable
      break;    
      
    case 10:
      aSample.setTable(KICKTEN_DATA); // change the current wavetable
      CURRENT_TABLE = KICKTEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;
  }
