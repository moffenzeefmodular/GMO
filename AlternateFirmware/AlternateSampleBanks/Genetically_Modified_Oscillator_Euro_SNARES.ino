...
Sample <SNAREONE_NUM_CELLS, AUDIO_RATE> aSample(SNAREONE_DATA); // create the wavetable template 
...
/* 
If the looping switch is on, or the loopingCv input jack recieves anything over +2 volts, 
...
*/
  ...
  */
  ...
  pitch = mapfloat(pitchMath, 1, 1023, 0.01, 20.0); // map from 0.01% to 20x speed 
  ...
  */
  ...
  /* 
  map from 1 to 11 (the number of sound files loaded + 1 to account for 0). If adding more files into the sketch, 
  change the last number of this map function to the total number of files + 1.
  */
  
  sampleSelect = map(sampleMath, 1, 1023, 1, 11); 
  
  /* Use a switch case statement to hot swap between wavetables. 
  If you've added files to this sketch, you must add a new case for each  
  new file in the following statement. Furthermore you must adjust the last number of the map function
  above to +1 the total number of files (as a space holder for 0).  
  */
  
  switch (sampleSelect) {
    case 1:
      aSample.setTable(SNAREONE_DATA); // change the current wavetable
      CURRENT_TABLE = SNAREONE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 2:
      aSample.setTable(SNARETWO_DATA); // change the current wavetable
      CURRENT_TABLE = SNARETWO_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 3:
      aSample.setTable(SNARETHREE_DATA); // change the current wavetable
      CURRENT_TABLE = SNARETHREE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 4:
      aSample.setTable(SNAREFOUR_DATA); // change the current wavetable
      CURRENT_TABLE = SNAREFOUR_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 5:
      aSample.setTable(SNAREFIVE_DATA); // change the current wavetable
      CURRENT_TABLE = SNAREFIVE_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 6:
      aSample.setTable(SNARESIX_DATA); // change the current wavetable
      CURRENT_TABLE = SNARESIX_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 7:
      aSample.setTable(SNARESEVEN_DATA); // change the current wavetable
      CURRENT_TABLE = SNARESEVEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 8:
      aSample.setTable(SNAREEIGHT_DATA); // change the current wavetable
      CURRENT_TABLE = SNAREEIGHT_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 9:
      aSample.setTable(SNARENINE_DATA); // change the current wavetable
      CURRENT_TABLE = SNARENINE_NUM_CELLS; // set the wavetable length to current wavetable
      break;
         
    case 10:
      aSample.setTable(SNARETEN_DATA); // change the current wavetable
      CURRENT_TABLE = SNARETEN_NUM_CELLS; // set the wavetable length to current wavetable
      break; 
