...
#include <samples/wavetables/rfiftyeOne_int8.h> // Table for Sample
#include <samples/wavetables/rfiftyeTwo_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeThree_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeFour_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeFive_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeSix_int8.h> // Table for Sample
#include <samples/wavetables/rfiftyeSeven_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeEight_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeNine_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeTen_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeEleven_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeTwelve_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeThirteen_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeFourteen_int8.h> // Table for Sample 
#include <samples/wavetables/rfiftyeFifteen_int8.h> // Table for Sample 

#define CONTROL_RATE 256 // powers of 2 please 

Sample <RFIFTYEONE_NUM_CELLS, AUDIO_RATE> aSample(RFIFTYEONE_DATA); // create the wavetable template 
...
*/
  ...
  */
  ...
  pitch = mapfloat(pitchMath, 1, 1023, 1.0, 20.0); // map from 0.01% to triple speed
  ...
  */
  ...
  switch (sampleSelect) {
    case 1:
      aSample.setTable(RFIFTYEONE_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYEONE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 2:
      aSample.setTable(RFIFTYETWO_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYETWO_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 3:
      aSample.setTable(RFIFTYETHREE_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYETHREE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 4:
      aSample.setTable(RFIFTYEFOUR_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYEFOUR_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 5:
      aSample.setTable(RFIFTYEFIVE_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYEFIVE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 6:
      aSample.setTable(RFIFTYESIX_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYESIX_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 7:
      aSample.setTable(RFIFTYESEVEN_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYESEVEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;
    
    case 8:
      aSample.setTable(RFIFTYEIGHT_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYEIGHT_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 9:
      aSample.setTable(RFIFTYNINE_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYNINE_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 10:
      aSample.setTable(RFIFTYTEN_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYTEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 11:
      aSample.setTable(RFIFTYEELEVEN_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYEELEVEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;
            
    case 12:
      aSample.setTable(RFIFTYETWELVE_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYETWELVE_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 13:
      aSample.setTable(RFIFTYETHIRTEEN_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYETHIRTEEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 14:
      aSample.setTable(RFIFTYEFOURTEEN_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYEFOURTEEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 15:
      aSample.setTable(RFIFTYEFIFTEEN_DATA); // change the current wavetable
      CURRENT_TABLE = RFIFTYEFIFTEEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;
