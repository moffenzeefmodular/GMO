...
#include <samples/wavetables/threeohoneOne_int8.h> // Table for Sample
#include <samples/wavetables/threeohoneTwo_int8.h> // Table for Sample 
#include <samples/wavetables/threeohoneThree_int8.h> // Table for Sample 
#include <samples/wavetables/threeohoneFour_int8.h> // Table for Sample 
#include <samples/wavetables/threeohoneFive_int8.h> // Table for Sample
#include <samples/wavetables/threeohoneSix_int8.h> // Table for Sample 
#include <samples/wavetables/threeohoneSeven_int8.h> // Table for Sample 

#define CONTROL_RATE 256 // powers of 2 please 

Sample <THREEOHONEONE_NUM_CELLS, AUDIO_RATE> aSample(THREEOHONEONE_DATA); // create the wavetable template 
...
*/
  ...
  */
  ...
  pitch = mapfloat(pitchMath, 1, 1023, 3.0, 15.0); // map from 0.01% to triple speed
  ...
  */
  ...
  sampleSelect = map(sampleMath, 1, 1023, 1, 8); 
  ...
  */
  ...
  switch (sampleSelect) {
    case 1:
      aSample.setTable(THREEOHONEONE_DATA); // change the current wavetable
      CURRENT_TABLE = THREEOHONEONE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 2:
      aSample.setTable(THREEOHONETWO_DATA); // change the current wavetable
      CURRENT_TABLE = THREEOHONETWO_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 3:
      aSample.setTable(THREEOHONETHREE_DATA); // change the current wavetable
      CURRENT_TABLE = THREEOHONETHREE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 4:
      aSample.setTable(THREEOHONEFOUR_DATA); // change the current wavetable
      CURRENT_TABLE = THREEOHONEFOUR_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 5:
      aSample.setTable(THREEOHONEFIVE_DATA); // change the current wavetable
      CURRENT_TABLE = THREEOHONEFIVE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 6:
      aSample.setTable(THREEOHONESIX_DATA); // change the current wavetable
      CURRENT_TABLE = THREEOHONESIX_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 7:
      aSample.setTable(THREEOHONESEVEN_DATA); // change the current wavetable
      CURRENT_TABLE = THREEOHONESEVEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;
