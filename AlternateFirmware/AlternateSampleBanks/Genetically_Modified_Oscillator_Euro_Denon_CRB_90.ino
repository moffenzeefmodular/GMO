...
#include <samples/wavetables/crbninetyOne_int8.h> // Table for Sample
#include <samples/wavetables/crbninetyTwo_int8.h> // Table for Sample 
#include <samples/wavetables/crbninetyThree_int8.h> // Table for Sample 
#include <samples/wavetables/crbninetyFour_int8.h> // Table for Sample 
#include <samples/wavetables/crbninetyFive_int8.h> // Table for Sample
#include <samples/wavetables/crbninetySix_int8.h> // Table for Sample 
#include <samples/wavetables/crbninetySeven_int8.h> // Table for Sample 


#define CONTROL_RATE 256 // powers of 2 please 

Sample <CRBNINETYONE_NUM_CELLS, AUDIO_RATE> aSample(CRBNINETYONE_DATA); // create the wavetable template 
...
*/
  ...
  */
  ...
  pitch = mapfloat(pitchMath, 1, 1023, 1.0, 10.0); // map from 0.01% to triple speed
  ...
  */
  ...
  switch (sampleSelect) {
    case 1:
      aSample.setTable(CRBNINETYONE_DATA); // change the current wavetable
      CURRENT_TABLE = CRBNINETYONE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 2:
      aSample.setTable(CRBNINETYTWO_DATA); // change the current wavetable
      CURRENT_TABLE = CRBNINETYTWO_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 3:
      aSample.setTable(CRBNINETYTHREE_DATA); // change the current wavetable
      CURRENT_TABLE = CRBNINETYTHREE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 4:
      aSample.setTable(CRBNINETYFOUR_DATA); // change the current wavetable
      CURRENT_TABLE = CRBNINETYFOUR_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 5:
      aSample.setTable(CRBNINETYFIVE_DATA); // change the current wavetable
      CURRENT_TABLE = CRBNINETYFIVE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 6:
      aSample.setTable(CRBNINETYSIX_DATA); // change the current wavetable
      CURRENT_TABLE = CRBNINETYSIX_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 7:
      aSample.setTable(CRBNINETYSEVEN_DATA); // change the current wavetable
      CURRENT_TABLE = CRBNINETYSEVEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;
