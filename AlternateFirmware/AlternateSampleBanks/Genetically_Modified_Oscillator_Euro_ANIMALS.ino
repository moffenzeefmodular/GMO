...
#include <samples/Wavetables/animalone_int8.h> // Table for Sample
#include <samples/Wavetables/animaltwo_int8.h> // Table for Sample 
#include <samples/Wavetables/animalthree_int8.h> // Table for Sample 
#include <samples/Wavetables/animalfour_int8.h> // Table for Sample 
#include <samples/Wavetables/animalfive_int8.h> // Table for Sample 
#include <samples/Wavetables/animalsix_int8.h> // Table for Sample 
#include <samples/Wavetables/animalseven_int8.h> // Table for Sample 
#include <samples/Wavetables/animaleight_int8.h> // Table for Sample 

#define CONTROL_RATE 256 // powers of 2 please 

Sample <ANIMALONE_NUM_CELLS, AUDIO_RATE> aSample(ANIMALONE_DATA); // create the wavetable template 
...
int lastTrigState = 0; // varaible to hold previous trig state 
...
*/
  ...
  */
  ...
  {
  pitchMath = (pot3Raw - startCvRaw); // So we add it to the Pot  
  }
  else if (startCvRaw < 512) // If the reading is below 512 we know it's a negatie value 
  {
  pitchMath = (pot3Raw + startCvRaw);  // So we subtract it from the Pot
  }
  ...
  */
  ...
  switch (sampleSelect) {
    case 1:
      aSample.setTable(ANIMALONE_DATA); // change the current wavetable
      CURRENT_TABLE = ANIMALONE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 2:
      aSample.setTable(ANIMALTWO_DATA); // change the current wavetable
      CURRENT_TABLE = ANIMALTWO_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 3:
      aSample.setTable(ANIMALTHREE_DATA); // change the current wavetable
      CURRENT_TABLE = ANIMALTHREE_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 4:
      aSample.setTable(ANIMALFOUR_DATA); // change the current wavetable
      CURRENT_TABLE = ANIMALFOUR_NUM_CELLS; // set the wavetable length to current wavetable
      break;

    case 5:
      aSample.setTable(ANIMALFIVE_DATA); // change the current wavetable
      CURRENT_TABLE = ANIMALFIVE_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 6:
      aSample.setTable(ANIMALSIX_DATA); // change the current wavetable
      CURRENT_TABLE = ANIMALSIX_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 7:
      aSample.setTable(ANIMALSEVEN_DATA); // change the current wavetable
      CURRENT_TABLE = ANIMALSEVEN_NUM_CELLS; // set the wavetable length to current wavetable
      break;
      
    case 8:
      aSample.setTable(ANIMALEIGHT_DATA); // change the current wavetable
      CURRENT_TABLE = ANIMALEIGHT_NUM_CELLS; // set the wavetable length to current wavetable
      break;
