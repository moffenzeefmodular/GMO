...
StateVariable <HIGHPASS> highpass1; // can be LOWPASS, BANDPASS, HIGHPASS or NOTCH
StateVariable <HIGHPASS> highpass2; // can be LOWPASS, BANDPASS, HIGHPASS or NOTCH
...
void updateControl(){
 ...
 ///////////////////////////////////////////
  ...
  decayOne = map(decayOneMath, 1, 1023, 0, 2000); // Map to full range of exponential attack/decay envelope. Range is 0-255 bits
  decayTwo = map(decayTwoMath, 1, 1023, 0, 2000);// Map to full range of exponential attack/decay envelope. Range is 0-255 bits
  }
  
  else {
  decayOne = map(decayOneMath, 1, 1023, 0, 100); // Map to full range of exponential attack/decay envelope. Range is 0-255 bits
  decayTwo = map(decayTwoMath, 1, 1023, 0, 100);// Map to full range of exponential attack/decay envelope. Range is 0-255 bits
  }
  ...
  */
  ...
  pitchOne = mapfloat(pitchOneMath, 1, 1023, 0.01, 1.0); // map from 0.01% to triple speed. 
  ...
  */
  ...
  pitchTwo = mapfloat(pitchTwoMath, 1, 1023, 0.01, 1.0); // Map from 0.01% to triple speed 
 ...
 ////////////////////////////////////////////////////////////////////////////////////
  ...
  highpassOne = map(pitchOneMath, 1, 1023, 80, 3000); 
  highpassTwo = map(pitchTwoMath, 1, 1023, 80, 3000);
  
  highpass1.setCentreFreq(highpassOne);
  highpass1.setResonance(200);   
  
  highpass2.setCentreFreq(highpassTwo);
  highpass2.setResonance(200);
 ...
 */
  return ((gainOne * (highpass1.next(aNoise.next()))) + (gainTwo * (highpass2.next(bNoise.next())))) * 5;
}
